"""REPL commands for GUI learning and recipe management.
...
async def handle_learn_command(args: str) -> str:
    ...
    Usage:
        /learn                      - Show help
        /learn <appname>            - Learn application interactively
        /learn <appname> --passive  - Learn via automatic traversal
        /learn list                 - List learned applications
        /learn show <appname>       - Show recipe details
        /learn rebuild <appname>    - Force rebuild recipe
        /learn delete <appname>     - Delete recipe
        /learn export <appname>     - Export recipe as JSON
        /learn import <file>        - Import recipe from JSON
...
def _get_help() -> str:
    ...
    return """
/learn - GUI automation learning commands

Usage:
  /learn <appname>           Learn an application (passive mode)
  /learn <appname> --passive Learn via automatic tree traversal
  /learn <appname> --interactive  Learn interactively (future)

  /learn list                List learned applications
  /learn show <appname>      Show recipe details
  /learn rebuild <appname>   Force rebuild recipe
  /learn delete <appname>    Delete a recipe
  /learn export <appname>    Export recipe as JSON
  /learn import <file>       Import recipe from JSON

Examples:
  /learn gnome-control-center    Learn GNOME Settings
  /learn show gnome-control-center   Show recipe details
  /learn list                    List all learned apps
...
async def _handle_learn(args: str) -> str:
    ...
    if not app_name or app_name.startswith("--"):
        return "Usage: /learn <appname> [--passive|--interactive]"
    ...
    try:
        ...
        logger.error(f"Error learning application: {e}")
        return f"Error learning application: {e}"
...
async def _handle_list(args: str) -> str:
    ...
    try:
        ...
        if not recipes:
            return "No learned applications. Use '/learn <appname>' to learn one."

        lines = ["Learned applications:\n"]
...
async def _handle_show(args: str) -> str:
    ...
    app_name = args.strip()
    if not app_name:
        return "Usage: /learn show <appname>"
    ...
    try:
        ...
        lines = [
            ...
            f"  Version: {recipe.version}",
            f"  Learn method: {recipe.learn_method}",
...
async def _handle_rebuild(args: str) -> str:
    ...
    app_name = args.strip()
    if not app_name:
        return "Usage: /learn rebuild <appname>"

    # Check existing recipe
    existing = get_recipe_by_app(app_name)
    if not existing:
        return f"No existing recipe for '{app_name}'. Use '/learn {app_name}' instead."
...
async def _handle_delete(args: str) -> str:
    ...
    app_name = args.strip()
    if not app_name:
        return "Usage: /learn delete <appname>"
...
async def _handle_export(args: str) -> str:
    ...
    app_name = args.strip()
    if not app_name:
        return "Usage: /learn export <appname>"
...
async def _handle_import(args: str) -> str:
    ...
    filepath = args.strip()
    if not filepath:
        return "Usage: /learn import <filepath>"
...
def handle_learn_command_sync(args: str) -> str:
    ...
    return loop.run_until_complete(handle_learn_command(args))
