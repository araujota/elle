...
Rule-based classifier that uses:
1. Hard keyword routing for meta/navigation commands
2. Prefix commands for explicit intent markers (/ask, /do, /sh, etc.)
3. Pattern matching for domain-specific requests
4. Default routing to agent loop for natural language inputs

All natural language understanding is delegated to the agent loop,
which uses the LLM for reasoning. The classifier only handles
routing and obvious pattern matching.
"""
...
# =============================================================================
# Hard routes - exact keyword matching
# =============================================================================
...
class IntentClassifier:
    ...
    3. Pattern-based classification (shell commands, questions, tasks)
    4. Default to SYSTEM_TASK (let the agent loop handle it)
    ...
    def classify(self, text: str, session: Session) -> IntentResult:
        ...
        if result and result.confidence >= MEDIUM_CONFIDENCE:
            # Apply safety overrides to pattern results
            result = self._apply_safety_overrides(text, result)
            self._log_classification(text, result, session)
            return result

        # Step 4: Default to agent loop (SYSTEM_TASK for actions, SYSTEM_QUESTION for questions)
        result = self._default_classification(text, session)
        result = self._apply_safety_overrides(text, result)
        self._log_classification(text, result, session)
        return result
    ...
    def _check_patterns(self, text: str, session: Session) -> IntentResult | None:
        ...
        # Question patterns (medium confidence - may need agent reasoning)
        ...
        # Task patterns (medium confidence - may need agent reasoning)
    ...
    def _default_classification(self, text: str, session: Session) -> IntentResult:
        """Default classification for unmatched input.

        Routes to SYSTEM_TASK by default - the agent loop will determine
        if it's a question or a task and handle appropriately.
        """
        ...
        if text.strip().endswith("?"):
            return IntentResult(
                intent=Intent.SYSTEM_QUESTION,
                confidence=0.75,
                rationale="Natural language question (ends with ?)",
                classified_by="rule",
            )

        # Default to SYSTEM_TASK - let the agent loop figure it out
        return IntentResult(
            intent=Intent.SYSTEM_TASK,
            confidence=0.70,
            rationale="Natural language input routed to agent loop",
            classified_by="rule",
        )
