...
def _generate_environment_file(
    key_path: str = "/etc/elle/db.key",
    env_path: str = "/etc/elle/environment",
) -> None:
    """Generate /etc/elle/environment with the ELLE secret key.

    This populates the ``ELLE_SECRET_KEY`` env var that the systemd
    ``EnvironmentFile=-/etc/elle/environment`` directive references.
    Silently skips if the key file does not exist or the environment
    file cannot be written (e.g., no root privileges).

    Args:
        key_path: Path to the encryption key file.
        env_path: Path to the environment file to write.
    """
    try:
        key_material = Path(key_path).read_text().strip()
    except (OSError, FileNotFoundError):
        logger.debug("Encryption key not found at %s; skipping environment file", key_path)
        return
    ...
    content = (
        "# /etc/elle/environment\n"
        "# Generated by elle setup — sourced by systemd EnvironmentFile\n"
        f"ELLE_SECRET_KEY={key_material}\n"
    )
    ...
    try:
        ...
        logger.info("Wrote environment file %s", env_path)
    except OSError as exc:
        logger.debug("Cannot write environment file %s: %s", env_path, exc)
...
class SetupWizard:
    ...
    def _save_configuration(self) -> None:
        ...
        # Generate /etc/elle/environment for systemd EnvironmentFile
            ...
            try:
                ...
                try:
                    ...
                    if process.returncode != 0:
                        logger.warning("Ollama serve exited with code %d", process.returncode)
                except subprocess.TimeoutExpired:
                    pass  # Expected — background daemon is still running
                return self._wait_for_ollama(timeout=30)
            except Exception as e:
        ...
        if telemetryd_path:
            try:
                ...
                try:
                    process.wait(timeout=5)
                    if process.returncode != 0:
                        logger.warning("Telemetryd exited with code %d", process.returncode)
                except subprocess.TimeoutExpired:
                    pass  # Expected — background daemon is still running
                time.sleep(2)
                if self._check_telemetryd_running():
        ...
        if not elled_path:
            ...
            try:
                ...
                try:
                    process.wait(timeout=5)
                    if process.returncode != 0:
                        logger.warning("Daemon (module) exited with code %d", process.returncode)
                except subprocess.TimeoutExpired:
                    pass  # Expected — background daemon is still running
        ...
        if elled_path:
            try:
                ...
                try:
                    process.wait(timeout=5)
                    if process.returncode != 0:
                        logger.warning("Daemon (elled) exited with code %d", process.returncode)
                except subprocess.TimeoutExpired:
                    pass  # Expected — background daemon is still running
                time.sleep(2)
                if self._check_daemon_running():
