...
def upsert_doc(conn: sqlite3.Connection | None, doc: ManDoc) -> int:
    """Insert or update a document.

    Uses UPSERT to handle both insert and update cases.

    Args:
        conn: SQLite connection (optional, will create if None).
...
def get_doc_by_id(conn: sqlite3.Connection | None, doc_id: int) -> ManDoc | None:
...
def get_all_hashes(conn: sqlite3.Connection | None) -> dict[tuple[str, str, str], str]:
...
def delete_doc(conn: sqlite3.Connection | None, doc_id: int) -> bool:
...
def upsert_chunk(conn: sqlite3.Connection | None, chunk: ManChunk) -> int:
...
def upsert_chunks_batch(conn: sqlite3.Connection | None, chunks: list[ManChunk]) -> int:
    """Batch upsert chunks.
...
def get_chunks_for_doc(conn: sqlite3.Connection | None, doc_id: int) -> list[ManChunk]:
...
def delete_chunks_for_doc(conn: sqlite3.Connection | None, doc_id: int) -> int:
...
def get_embedding(conn: sqlite3.Connection | None, chunk_id: int) -> list[float] | None:
...
def get_chunks_without_embeddings(conn: sqlite3.Connection | None, limit: int = 1000) -> list[ManChunk]:
...
def count_docs(conn: sqlite3.Connection | None) -> int:
...
def count_chunks(conn: sqlite3.Connection | None) -> int:
    """Count total chunks.
...
def count_embeddings(conn: sqlite3.Connection | None) -> int:
...
def get_section_counts(conn: sqlite3.Connection | None) -> dict[str, int]:
...
def get_meta(conn: sqlite3.Connection | None, key: str) -> str | None:
