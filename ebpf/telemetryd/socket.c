...
/**
 * socket.c - Unix socket management for elled-telemetryd
 *
...
static void accept_clients(struct telem_socket *sock)
{
    while (1) {
...
{
    return sock ? sock->listen_fd : -1;
}

int telem_socket_num_clients(struct telem_socket *sock)
{
    return sock ? sock->num_clients : 0;
}
