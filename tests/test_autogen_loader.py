...
class TestCompileCoreCapability:
    def test_compile_finds_class_by_name(self):
        ...
        stored_mut = StoredCapability(**{**stored.model_dump(), "capability_class_code": code})
    ...
    def test_compile_syntax_error(self):
        stored = _make_stored()
        stored_mut = StoredCapability(**{**stored.model_dump(), "capability_class_code": "def ===INVALID"})
        result = _compile_core_capability(stored_mut)
        assert result is None
...
class TestLoadAllApproved:
    def test_loads_and_registers(self):
        ...
        with (
            patch("elle.capabilities.autogen.loader.get_store", return_value=mock_store),
            patch(
...
class TestLoadCapabilityByName:
    ...
    def test_not_found(self):
        ...
        with patch("elle.capabilities.autogen.loader.get_store", return_value=mock_store):
...
class TestRegisterAutogenCapabilities:
    def test_delegates_to_load_all_approved(self):
        mock_registry = MagicMock()
        with patch("elle.capabilities.autogen.loader.load_all_approved", return_value=5) as mock_load:
...
class TestGetLoadedAutogenCapabilities:
    ...
    def test_skips_on_error(self):
        ...
        stored_bad = StoredCapability(**{**stored.model_dump(), "spec_json": "INVALID"})
        mock_store = MagicMock()
